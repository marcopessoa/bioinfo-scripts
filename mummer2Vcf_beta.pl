...
# 2016-06-14: Marco Pessoa <marco.pessoa@embrapa.br> modified the script to work
#             with the 'show-snps -CT' command. This avoids alignments in repeats
#             and is a recommended option in MUMmer.
# 2016-06-14: Marco Pessoa <marco.pessoa@embrapa.br> also changed the resulting VCF 
#             header line and resulting columns to the 8 mandatory columns defined 
#             by VCF version 4.0.   
...
sub complete_snp($$$$) {
    if ($o_type ne "INDEL") {
        ...
        print STDOUT join("\t", $ref, $pos, "s\_".$pos, $orig, $alt, ".", ".", "."), "\n";
    }
}
...
while (<>) {
    ...
    if( $line[1] eq "." || $line[2] eq ".") {
        ...
        complete_snp($line[8], $line[0], $line[1], $line[2]);
    }
}
complete_indel() if $indel;
